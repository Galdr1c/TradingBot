...
import {
  ...
  ReferenceLine, ComposedChart, Bar, LineChart, Line
} from 'recharts';
...
export default function Charts() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // BUG-FIX: getHistory returns { data:[], signal:{} } — NOT an array
      const [histRes, sig] = await Promise.all([
      ...
      setData(rows.map(d => ({
        ...
        p:        d.close,
        o:        d.open,  h: d.high,  l: d.low,  v: d.volume,
        rsi:      d.rsi,
        macd:     d.macd,     macd_s: d.macd_signal, macd_h: d.macd_hist,
        bb_u:     d.bb_upper, bb_m:   d.bb_mid,      bb_l:   d.bb_lower,
        ema20:    d.ema_20,   ema50:  d.ema_50,       ema200: d.ema_200,
        adx:      d.adx,
        williams: d.williams_r,
        stoch:    d.stoch_k,
        vwap:     d.vwap,
        obv:      d.obv,
        candlePattern: d.candle_pattern,
        candleSignal: d.candle_signal,
        candleScore: d.candle_score,
      })));
      setSignal(sig);
    ...
    <div className="col fade-in" style={{ gap:10 }}>
      ...
      <div className="panel">
        <div style={{ padding:'8px 14px', display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
          ...
          <div style={{ display:'flex', gap:4 }}>
            {TIMEFRAMES.map(t => (
              <button key={t} className="btn sm" onClick={() => setTf(t)}
          ...
          <div style={{ marginLeft:'auto', display:'flex', gap:16, alignItems:'center' }}>
            ...
            {signal && (
              <span style={{ fontFamily:'Orbitron,sans-serif', fontSize:11, fontWeight:700, color:sigColor }}>
      ...
      <div style={{ display:'flex', gap:8, overflowX:'auto', paddingBottom:2 }}>
        {[
          ...
          { label:'MACD HIST',  value: last.macd_h?.toFixed(4),    color: last.macd_h > 0 ? 'var(--green)' : 'var(--red)' },
          { label:'ADX',        value: last.adx?.toFixed(1),       color: last.adx > 25 ? 'var(--orange)' : 'var(--text2)' },
          { label:'W%R',        value: last.williams?.toFixed(0),  color: last.williams < -80 ? 'var(--green)' : last.williams > -20 ? 'var(--red)' : 'var(--text2)' },
          { label:'EMA 20',     value: last.ema20?.toFixed(2),     color:'var(--cyan)' },
          { label:'EMA 50',     value: last.ema50?.toFixed(2),     color:'var(--purple)' },
          { label:'EMA 200',    value: last.ema200?.toFixed(2),    color:'var(--orange)' },
          { label:'STOCH K',    value: last.stoch?.toFixed(1),     color:'var(--text2)' },
          { label:'VWAP',       value: last.vwap ? `$${fmt(last.vwap)}` : null, color:'var(--orange)' },
          ...
          {signal?.reasons?.length > 0 && (
            <div style={{ marginLeft:'auto', display:'flex', gap:6, alignItems:'center', paddingRight:14 }}>
              {signal.reasons.slice(0,2).map((r,i) => (
                <span key={i} style={{ fontSize:8, color:'var(--text2)', background:'rgba(255,255,255,0.03)',
                  padding:'2px 7px', borderRadius:2, border:'1px solid var(--border)' }}>{r}</span>
              ))}
            </div>
          )}
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {tab === 'price' ? (
              <AreaChart data={data} margin={{ top:4, right:4, left:0, bottom:0 }}>
                ...
                {/* VWAP line */}
                {last.vwap && <ReferenceLine y={last.vwap} stroke="rgba(255,224,0,0.4)" strokeDasharray="4 4" label={{ value:'VWAP', position:'right', fontSize:8, fill:'var(--yellow)' }} />}
                ...
                <Area type="monotone" dataKey="ema200" stroke="rgba(255,100,0,0.35)"  strokeWidth={1}   fill="none" dot={false} />
              ...
              <AreaChart data={data} margin={{ top:4, right:4, left:0, bottom:0 }}>
                ...
                <Tooltip contentStyle={{ background:'#040810', border:'1px solid #0f2040', fontSize:10, borderRadius:4 }} />
                <ReferenceLine y={70} stroke="rgba(255,45,85,0.45)"  strokeDasharray="4 4" label={{ value:'70', position:'right', fontSize:8, fill:'var(--red)' }} />
                <ReferenceLine y={50} stroke="rgba(255,255,255,0.1)" strokeDasharray="2 4" />
                <ReferenceLine y={30} stroke="rgba(0,232,122,0.45)"  strokeDasharray="4 4" label={{ value:'30', position:'right', fontSize:8, fill:'var(--green)' }} />
                <Area type="monotone" dataKey="rsi" stroke="var(--purple)" strokeWidth={1.5} fill="url(#grsi)" dot={false} />
              </AreaChart>
            ) : tab === 'macd' ? (
              <ComposedChart data={data} margin={{ top:4, right:4, left:0, bottom:0 }}>
                <XAxis dataKey="t" tick={{ fontSize:8, fill:'var(--text2)' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fontSize:8, fill:'var(--text2)' }} tickLine={false} axisLine={false} width={45} />
                <Tooltip contentStyle={{ background:'#040810', border:'1px solid #0f2040', fontSize:10, borderRadius:4 }} />
                <ReferenceLine y={0} stroke="rgba(255,255,255,0.1)" />
                <Bar dataKey="macd_h" fill="var(--cyan)" opacity={0.55} />
                <Line type="monotone" dataKey="macd"   stroke="var(--cyan)"   strokeWidth={1.5} dot={false} />
                <Line type="monotone" dataKey="macd_s" stroke="var(--orange)" strokeWidth={1.2} dot={false} />
              </ComposedChart>
            ) : tab === 'adx' ? (
              <LineChart data={data} margin={{ top:4, right:4, left:0, bottom:0 }}>
                ...
                <YAxis domain={[0,100]} tick={{ fontSize:8, fill:'var(--text2)' }} tickLine={false} axisLine={false} width={30} />
                <Tooltip contentStyle={{ background:'#040810', border:'1px solid #0f2040', fontSize:10, borderRadius:4 }} />
                <ReferenceLine y={25} stroke="rgba(245,158,11,0.4)" strokeDasharray="4 4" label={{ value:'25 (trend)', position:'right', fontSize:8, fill:'var(--orange)' }} />
                <Line type="monotone" dataKey="adx"      stroke="var(--orange)" strokeWidth={1.8} dot={false} name="ADX" />
                <Line type="monotone" dataKey="williams"  stroke="var(--purple)" strokeWidth={1.2} dot={false} name="Williams %R" />
              </LineChart>
            ) : (
              <ComposedChart data={data} margin={{ top:4, right:4, left:0, bottom:0 }}>
                <XAxis dataKey="t" tick={{ fontSize:8, fill:'var(--text2)' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fontSize:8, fill:'var(--text2)' }} tickLine={false} axisLine={false} width={50}
        ...
        <div className="panel">
          ...
          <div style={{ padding:14, display:'flex', gap:20, alignItems:'flex-start' }}>
            <div style={{ textAlign:'center', minWidth:130 }}>
              ...
              <div style={{ fontSize:8.5, color:'var(--text2)', marginTop:4 }}>Score: {signal.score?.toFixed(4)}</div>
            </div>
            <div className="col" style={{ gap:5, flex:1 }}>
              <div style={{ fontSize:7.5, color:'var(--text2)', letterSpacing:1.5, marginBottom:2 }}>SIGNAL REASONS (weighted)</div>
              {(signal.reasons || []).map((r,i) => (
                <div key={i} style={{ display:'flex', gap:8, alignItems:'center', fontSize:10, color:'var(--text)' }}>
                  <span style={{ color:sigColor, fontSize:12 }}>›</span> {r}
                </div>
              ))}
            </div>
            {/* Indicator snapshot */}
            {signal.indicators && (
              <div className="col" style={{ gap:4, minWidth:160 }}>
                <div style={{ fontSize:7.5, color:'var(--text2)', letterSpacing:1.5, marginBottom:2 }}>INDICATOR SNAPSHOT</div>
                {Object.entries(signal.indicators).map(([k,v]) => (
                  <div key={k} style={{ display:'flex', justifyContent:'space-between', fontSize:9.5 }}>
                    <span style={{ color:'var(--text2)', textTransform:'uppercase' }}>{k.replace(/_/g,' ')}</span>
                    <span style={{ color:'var(--cyan)', fontFamily:'var(--font-mono)' }}>{v}</span>
                  </div>
                ))}
              </div>
            )}
