...
export default function Kronos() {
  ...
  const runForecast = useCallback(async () => {
    ...
    try {
      ...
      const histPoints = hist.slice(-40).map(d => ({
        t: d.timestamp ? new Date(d.timestamp).toLocaleTimeString('en',{hour:'2-digit',minute:'2-digit'}) : '',
        actual: d.close, forecast: null, hi: null, lo: null, isHist: true
      }));
      const fcPoints = (pred.forecast || []).map(d => ({
        t: d.timestamp ? new Date(d.timestamp).toLocaleTimeString('en',{hour:'2-digit',minute:'2-digit'}) : `+${d.t+1}h`,
        actual: null, forecast: d.p, hi: d.hi, lo: d.lo, isHist: false
      }));
      // Bridge point
    ...
    <div className="col fade-in" style={{ gap:10 }}>
      ...
      <div className="panel">
        <div style={{ padding:'12px 16px', display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div>
            <div style={{ fontFamily:'Orbitron,sans-serif', fontSize:11, fontWeight:700, color:'var(--text3)', letterSpacing:2 }}>KRONOS FOUNDATION MODEL</div>
            <div style={{ fontSize:9, color:'var(--text2)', marginTop:2 }}>Zero-shot financial time series forecasting · NeoQuasar/Kronos-small · AAAI 2026</div>
          </div>
        ...
        <div className="panel">
          ...
          <div className="fc-params">
            {[
              ...
              <div key={p.label} className="fc-param-row">
                <span className="fc-param-label">{p.label}</span>
                <span className="fc-param-val">{p.val}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="panel glow-cyan" style={{ flex:1, minHeight:320 }}>
        <div className="ph">
          ...
          {result && <span style={{ marginLeft:8, fontSize:8, color:'var(--text2)' }}>Model: {result.model} · Generated: {new Date(result.timestamp).toLocaleTimeString()}</span>}
        </div>
        ...
        {!result ? (
          <div className="loading-cell" style={{ flex:1, flexDirection:'column', gap:12 }}>
            ...
            <div style={{ fontSize:9, color:'var(--text2)', opacity:.7 }}>Kronos will analyze K-line patterns and forecast the next {horizon} bars</div>
          </div>
        ) : (
          <div style={{ flex:1, padding:'8px 4px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={history} margin={{top:4,right:4,left:0,bottom:0}}>
                ...
                <YAxis domain={['auto','auto']} tick={{fontSize:8,fill:'var(--text2)'}} tickLine={false} axisLine={false} width={65} tickFormatter={v=>v>1000?`$${(v/1000).toFixed(1)}k`:`$${v.toFixed(2)}`} />
                <Tooltip contentStyle={{background:'#040810',border:'1px solid #0f2040',fontSize:9.5,borderRadius:4}}
                  formatter={(v,n) => v ? [`$${fmt(v)}`, n==='actual'?'Price':n==='forecast'?'Kronos Forecast':n==='hi'?'Upper Band':'Lower Band'] : [null]} />
                {/* Divider at last actual */}
                <ReferenceLine x={history.filter(h=>h.isHist).slice(-1)[0]?.t} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 4" label={{ value:'NOW', position:'top', fontSize:8, fill:'var(--text2)' }} />
      ...
      {result && fc.length > 0 && (
        <div className="panel">
          ...
          <div className="sc" style={{ maxHeight:160 }}>
            <table className="tbl">
              <thead><tr><th>BAR</th><th>FORECAST PRICE</th><th>UPPER BAND</th><th>LOWER BAND</th><th>RANGE</th></tr></thead>
              <tbody>
                {fc.slice(0,12).map((f,i) => (
                  <tr key={i}>
                    <td style={{ color:'var(--text2)' }}>+{i+1}</td>
                    <td style={{ color:'var(--purple)', fontFamily:'Orbitron,sans-serif', fontSize:10 }}>${fmt(f.p)}</td>
                    <td style={{ color:'var(--text2)' }}>${fmt(f.hi)}</td>
                    <td style={{ color:'var(--text2)' }}>${fmt(f.lo)}</td>
                    <td style={{ color:'var(--orange)' }}>±${fmt((f.hi-f.p))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
